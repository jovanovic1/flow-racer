...
public class GameLauncher : MonoBehaviour, INetworkRunnerCallbacks
{
	...
	private List<SessionInfo> _currentSessionList = new List<SessionInfo>();
	...
	private void Start()
	{
		...
		_runner.AddCallbacks(this); //to register callbacks

		_pool = go.AddComponent<FusionObjectPoolRoot>();

		SceneManager.LoadScene(LevelManager.LOBBY_SCENE);

		//hardcoding for now, will be fetched from server
		if(!PlayerPrefs.HasKey("gFee"))
		{
			PlayerPrefs.SetFloat("gFee", 100f);
		}
	...
	public async Task<SessionInfo> GetSessionInfoByID()
	{
		Debug.Log("Getting session info by ID"+ _currentSessionList.Count);
		if (_runner == null)
		{
			return null;
		}
		...
		foreach (SessionInfo session in _currentSessionList)
		{
			if (session.Name == ClientInfo.LobbyName)
			{
				return session;
			}
		}
	...
	public void OnConnectFailed(NetworkRunner runner, NetAddress remoteAddress, NetConnectFailedReason reason)
	{
		...
		_popUp.SetDataAndOpen(status, message, "OK");
	}
	...
	{
		Debug.Log("Session list updated"+sessionList.Count);
		if(runner == null)
			return;
